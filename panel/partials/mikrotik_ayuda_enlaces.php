...
<div id="ayuda-enlaces-mikrotik" class="nc-card p-4 border-secondary border-opacity-25">
    ...
    <p class="small mb-2">Cuando el túnel queda <strong>activo</strong>, el servidor asigna al MK una <strong>IP privada dentro de la VPN del proveedor</strong> (la ves en el panel o en <code>IP → Addresses</code> sobre la interfaz del cliente VPN). Esa es la IP que cargás en el <strong>panel</strong> (MikroWisp “IP/Host”; en NetControl <strong>Routers → IP o host</strong>) junto con el <strong>usuario/contraseña API</strong> del RouterOS. Orden lógico: <em>primero VPN estable</em> → <em>después</em> alta en el panel con la IP del túnel.</p>
    <p class="small mb-2">En paneles tipo <strong>MikroWisp</strong> los routers remotos aparecen con la dirección que les da el <strong>servidor VPN</strong> del NOC. El software de gestión no “atraviesa” el Starlink hacia una IP pública del cliente; habla al MK por esa <strong>IP interna del túnel</strong>.</p>
    <ul class="small mb-3">
        <li><strong>IP / host en NetControl</strong> = mismo concepto que “IP/Host” en MikroWisp: la que usa el panel para <strong>API RouterOS</strong>. Si además usás el hub WireGuard de este repo, esa otra VPN usa <code>10.64.0.x</code> — no mezcles direcciones de una VPN con la de la otra.</li>
